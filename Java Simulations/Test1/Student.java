...
//             double bb= ((y_me-y_n1)*1.2/dist1)+(((1-(y_me-y_n1)*(y_me-y_n1))*(b)-(x_me-x_n1)*(y_me-y_n1)*(a))/(dist1*dist1))* 0.01;
             ...
             double x_temp_o= ((x_me-x_o)/dist_o)+(((1-(x_me-x_o)*(x_me-x_o))*(x_me-x_n2)-(x_me-x_o)*(y_me-y_o)*(y_me-y_n2))/(dist_o*dist_o));
             double y_temp_o= ((y_me-y_o)/dist_o)+(((1-(y_me-y_o)*(y_me-y_o))*(y_me-y_n2)-(x_me-x_o)*(y_me-y_o)*(x_me-x_n2))/(dist_o*dist_o));
             sumForces.addIn(new Double2D((x_temp_o) * students.GoToGoalMultiplier, (y_temp_o) * students.GoToGoalMultiplier));
             
             sumForces.addIn(new Double2D((x_temp_o) , (y_temp_o) ));
             }
