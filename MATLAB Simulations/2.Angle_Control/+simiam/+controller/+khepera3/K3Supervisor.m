...
%             fprintf('current_pose: (%0.3f,%0.3f,%0.3f)\n', x, y, theta);
        ...
        %   measured wheel encoder tick counts are stored in prev_ticks.
            ...
            %% START CODE BLOCK %%
            D_r=(right_ticks-prev_right_ticks)*m_per_tick; %distance travelled by right wheel
            D_l=(left_ticks-prev_left_ticks)*m_per_tick;    %distance travelled by left wheel
            D_c=(D_r+D_l)/2;    %distance travelled by centre
            x_dt = D_c*cos(theta);
            y_dt = D_c*sin(theta);
            theta_dt = (D_r-D_l)/L;
